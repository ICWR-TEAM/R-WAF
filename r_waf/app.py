...
def ensure_dirs_and_files(config):
    ...
    if not os.path.exists(banned_html_path):
        banned_html_content = open("ban.html", "r", encoding="utf-8").read() if os.path.exists("ban.html") else "403 Forbidden - Access Denied by R-WAF"
...
class WAFApp:
    def __init__(self, config):
        ...
        self.anti_ddos = config["anti_ddos"]
        self.ddos_window_seconds = config["ddos_window_seconds"]
        self.ddos_max_requests = config["ddos_max_requests"]
        self.ddos_hits = {}
    ...
    def anti_ddos_check(self, ip):
        now = datetime.now(timezone.utc)
        hits = self.ddos_hits.get(ip, [])
        hits = [t for t in hits if (now - t).total_seconds() <= self.ddos_window_seconds]
        hits.append(now)
        self.ddos_hits[ip] = hits
        if len(hits) > self.ddos_max_requests:
            logger.info(f"Blocked IP {ip} by DDOS protection")
            self.add_ban(ip, reason="ddos")
            return {"action": "block", "reason": "ddos"}
        return {"action": "allow"}
    ...
    def check_request(self, ip, header, user_agent, path, body=""):
        try:
            ddos_result = self.anti_ddos_check(ip)
